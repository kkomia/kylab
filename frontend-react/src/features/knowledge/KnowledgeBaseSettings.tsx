...
export function KnowledgeBaseSettings({ kb, className, onChanged }: KnowledgeBaseSettingsProps) {
  ...
  return (
    <span className={['kb-menu-wrap', className].filter(Boolean).join(' ')}>
      <IconButton
        icon={SettingsIcon}
        label={`${kb.name} 的设置`}
        title="知识库设置"
      ...
      <Modal
        ...
        onEnter={() => {
          if (SAVE_SECTIONS.includes(section)) void save()
        }}
        footer={
              ...
              <>
                ...
                <div className="kb-field">
                  ...
                  <div className="kb-field-inline">
                    ...
                    <Button size="sm" aria-label="复制知识库 ID" onClick={() => void copyId()}>
                      复制
                    </Button>
                  </div>
      ...
      <ConfirmDialog
        ...
        busy={reingesting}
        busyLabel="排队中…"
        onConfirm={() => void confirmReingest()}
