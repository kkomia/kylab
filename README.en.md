...
> [knowledge base product benchmark](docs/知识库产品对标调研-v0.1.md) and the
> [Agent workspace & capability layer design](docs/Agent-工作区与能力层设计-v0.1.md) (both Chinese):
...
> [development plan §12](docs/开发计划-v0.1.md), and `scripts/ci.*` is the way to reproduce it.
...
- **Chat and agent**: SSE streaming with smooth output, forced citations (`[n]` mapped to
  ...
  and page fetching, Office export (docx/xlsx/pptx/pdf) and sub-agent spawning are all tools,
  with what a tool may touch scoped by session and workspace; context is compressed
...
LAN access and deployment details: [deployment & operations v0.2](docs/部署与运行-v0.2.md) (Chinese).
...
|----------|----------|
| **Specs and plans** | |
| [Architecture v0.2](docs/架构设计-v0.2.md) | Original positioning, architecture, ingestion pipeline, retrieval, capacity planning. **Two parts are void**: the positioning (see the "drift" section above) and §8's storage choice (SQLite), superseded by PostgreSQL |
| [Engineering spec v0.3](docs/项目工程规范-v0.3.md) | Layout, naming, layering discipline, testing, document-freeze rule, commits and branches |
| [Frontend design spec v0.14](docs/前端设计规范-v0.14.md) | Design tokens, light/dark themes, accessible controls, UI information architecture (values measured from Kimi) |
| [API spec v0.1](docs/API-接口规范-v0.1.md) | REST routes, request/response conventions, error envelope (accepted when it matches the generated OpenAPI) |
| [Deployment & operations v0.2](docs/部署与运行-v0.2.md) | PostgreSQL prerequisite, local and container routes, backup/restore, capacity and pitfalls (v0.1 superseded, kept as an archive) |
| [Development plan v0.1](docs/开发计划-v0.1.md) | Milestones M0–M7, task breakdown, quality gates, risk register; §11–§12 log each iteration's gap absorption and what actually landed (the longest document) |
| [Handover 2026-09-11](docs/交接说明-2026-09-11.md) | Read this first when taking over: three commands to get running, discipline, pitfalls. **A snapshot of that day** — it still describes SQLite storage, so this README wins |
| **Design and research** | |
| [Agent workspace & capability layer v0.1](docs/Agent-工作区与能力层设计-v0.1.md) | Workspaces, skills and external MCP, tool admission policy, session sandboxes, sub-agents |
| [Memory layer design v0.1](docs/记忆层设计-v0.1.md) | Persona files and long-term memory (benchmarked against ReMe / QwenPaw), phases and acceptance |
...
| [Wiki generation research v0.1](docs/Wiki生成调研-v0.1.md) | Four routes for auto-generating wiki pages and a copy-ready checklist |
| [Skills repo & marketplace research v0.1](docs/技能仓库与技能市场调研-v0.1.md) | Open-source skill repos and third-party marketplaces (research only, no implementation) |
| [Thinking mode research v0.1](docs/思考模式调研-v0.1.md) | Provider dialects and effort levels for thinking mode (see `services/thinking.py`) |
| [Retrieval evaluation v0.1](docs/检索评测-v0.1.md) | Retrieval quality evaluation method and conclusions |
| [UI review & improvement plan v0.2](docs/界面评审与改进计划-v0.2.md) | Second round of UI review items (executed); the first round is v0.1 |
| [UI assessment v0.1](docs/UI-评估与优化方案-v0.1.md) | Measured performance, visual/layout assessment, and a feature comparison against Dify / MaxKB / WeKnora |
| [Kimi design token reference](docs/Kimi-设计token对照表.md) | The reference values behind the frontend art direction (measured from Kimi's stylesheet; not the spec itself) |
| **Archive** | |
| [Code review M2/M4 v0.1](docs/质检报告-M2M4-async-worker-rest-api-v0.1.md) | Milestone review of the async consumer and core REST API (verdict: pass) |
| [Code review M5 v0.1](docs/质检报告-M5-web-console-v0.1.md) | Milestone review of the web console (verdict: pass) |
