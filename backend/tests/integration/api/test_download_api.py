...
def client(monkeypatch):
    # 配一个签名密钥，这样"签发"这条路可用；同时因为配了凭据，鉴权会自动生效
    monkeypatch.setenv("KYLAB_URL_SIGNING_SECRET", SIGNING_SECRET)
    monkeypatch.setenv("KYLAB_CONSOLE_TOKEN", "console-token-for-download")
...
CONSOLE = {"Authorization": "Bearer console-token-for-download"}


@pytest.fixture
def document_id(client: TestClient) -> str:
    kb = client.post("/api/v1/knowledge-bases", json={"name": "下载测试库"}, headers=CONSOLE)
    kb_id = kb.json()["id"]
    upload = client.post(
...
def test_issuing_a_link_requires_credentials(client: TestClient, document_id: str) -> None:
...
def test_signed_link_is_short_lived(client: TestClient, document_id: str) -> None:
    body = client.get(
        f"/api/v1/documents/{document_id}/download-url", headers=CONSOLE
    ).json()
...
def test_signature_cannot_be_reused_for_another_document(
    ...
    other = client.post("/api/v1/knowledge-bases", json={"name": "另一个库"}, headers=CONSOLE)
