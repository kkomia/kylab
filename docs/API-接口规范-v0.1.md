...
**鉴权在满足任一条件时自动启用**：`KYLAB_AUTH_ENABLED=true`、
设置了 `KYLAB_CONSOLE_TOKEN`、或库里已存在任何 API Key。
未启用时所有接口开放（单机自用的默认形态）。

| 身份 | 令牌形式 | 能做什么 |
|------|---------|---------|
| **控制台** | `kylab_console_…` | 全部，**且只有它能读写 `/settings`、`/api-keys`、模型凭据、使用者名册** |
| **读写密钥** | `kylab_sk_…` | 读写内容（建库、上传、删文档、拉数据源），**但碰不到凭据** |
| **只读密钥** | `kylab_sk_…` | 只读；所有写操作返回 `403` |

三条纪律：

1. **凭据永不出接口**。密钥相关的响应只给"配没配"与掩码尾巴（`sk-…ec6`），
   任何情况下不回明文。明文**只在创建 API Key 的那一次响应里出现**。
2. **API Key 只存 SHA-256 哈希**，明文不落库。
3. **只读与读写密钥都能限定知识库范围**（`knowledge_base_ids`）——
   越权的请求返回 `403`，而不是"空的检索结果"。

> **名册不是鉴权。** 使用者名册（`/users`）只回答"是谁传的"，
...
| `GET` | `/api/v1/auth/status` | 控制台令牌是否已初始化 |
