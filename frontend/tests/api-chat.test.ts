/**
 * 对话协议层（`api/chat.ts` 的 `pump` / `chatStream`）——**从旧 Vue 版
 * `tests/unit/api/chat.test.ts` 整份搬来的**（实现同一份代码，只换了会话令牌的 import 路径）。
 *
 * 为什么必须留着它：新前端的其它用例都把这个模块 **mock 掉**了
 * （`chat-model-live-turn` 抓走 handlers 自己推事件、`chat-ui` 直接替掉整个模块），
 * 于是"**真的 SSE 文本进来、真的解析器跑一遍**"这条链路上没有任何兜底——
 * 而它是这次迁移动过的两个文件之一（`chat.ts` 补了工具步骤漏转发的 `kind`）。
 * 这一份钉的正是那些：凭据、事件解析、分片切在 JSON 中间、HTTP 错误、取消、
 * 缺 done、平滑模式的交付时机、审批事件、重连锚点与 `logSeq`、live 端点。
 *
