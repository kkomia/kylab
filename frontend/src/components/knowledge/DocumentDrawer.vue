...
/**
 * 文档详情抽屉：**从右侧滑出，盖在文档列表上**——列表样式一点不动。
 *
 * 为什么是抽屉而不是独立页面：一份 PDF 放在整页里只占左半边，右边全是空白；
 * 而"这份文档长什么样"本来就是**从列表里点开看一眼**的动作，看完就回去继续扫列表。
 * 抽屉让列表始终在左侧可见，翻完关掉就走，不必"回列表"再找位置。
 *
 * 内容分区参考 WeKnora 的文档侧栏：**基本信息**（上传时间 / 类型 / 大小 / 状态…）
 * 与**文件内容**（片段数 + 阅读/切块两个视角）。
 *
...
/** 把这一份文档的三个数据源（元信息 / 切块 / 阅读视角）一次加载齐。 */
 ...
 * 不会重新挂载——只写在 onMounted 里就会"点了没反应"。
 */
watch(
  () => props.documentId,
  (value) => {
...
<template>
  <!--
    抽屉：`position: fixed` 贴右边、整屏高，从右侧滑入，**盖在文档列表上**。
...
/* 分区标题：抽屉里的信息分两块（基本信息 / 文件内容），标题要能一眼分段 */
.section-title {
  display: flex;
  align-items: center;
  gap: var(--space-2);
...
/* 元信息装进一块圆角面板：它是"这份文档的身份证"，不是散落的标签 */
.meta {
  display: grid;
