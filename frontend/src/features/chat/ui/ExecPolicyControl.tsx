/**
 * 输入框那一排的「执行策略」（旧 `components/chat/ExecPolicyControl.vue`）。
 *
...
export function ExecPolicyControl() {
  ...
  async function choose(value: string): Promise<void> {
    ...
    try {
      ...
      notifySuccess(`执行策略已改成「${chosen?.label ?? value}」，下一个动作就生效`)
    } catch (cause) {
      notifyError(cause)
    } finally {
      setSaving(false)
  ...
  return (
    <Dropdown label={`执行·${label}`} ariaLabel="执行策略" icon={<ShieldCheck size={14} />}>
      {POLICIES.map((item) => (
        <button
          key={item.value}
          type="button"
