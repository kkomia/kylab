  ...
  return (
    <div className={PANEL} role="listbox" aria-label="命令">
      <div ref={listRef} className={LIST}>
        ...
        {flat.length === 0 ? <p className={EMPTY}>没有匹配的命令</p> : null}
      </div>
      {/* 脚注只留键盘提示：原来还缀着"命令由后端直接执行，不进模型历史"——
          那是这一层怎么运作，不是眼下这一下按哪儿 */}
      <p className={FOOT}>↑↓ 选择，回车或点击插入</p>
    </div>
  )
}
...
export function MentionMenu({
  ...
  return (
    <div className={PANEL} role="listbox" aria-label="添加上下文">
      ...
      {/*
        脚注只留键盘提示。原来那一段（"知识库那一条是并进本轮检索范围（点一下生效，
        在输入框的「知识库」里取消）；文件 / 技能 / 会话只插入引用、不读取内容"）
        是把四类条目各自的行为讲一遍——2026-09-24 按用户要求删：条目自己写着名字，
        点一下就看到结果，不需要先读一段说明书。
      */}
      <p className={FOOT}>↑↓ 选择，回车或点击生效</p>
    </div>
  )
}
