/**
 * 记忆图谱的布局（纯函数，从旧前端 `components/memory/graphLayout.ts` 逐字搬来）。
 *
 * **确定性、没有物理引擎**：不引第三方图库、不做力导向迭代。三条理由：
 * 1. 力导向每次跑出来都不一样——同一份记忆刷新两次是两个形状，用户记不住，
 *    也没法用它比对"我改完之后结构变了没有"；
 * 2. 这张图的规模是几十个节点（工作区里的 .md），不是几万个，不需要退化布局；
 * 3. 可测：给定一份输入就一定能算出同一组坐标。
 *
 * 算法：按无向连通分量切开，每个分量从**度数最大的节点**（枢纽）出发做 BFS 分层；
 * 第 0 层放中心，第 n 层放第 n 个环上，同层均匀铺开。分量按格子横向排开。
 *
 * **坐标空间就是像素空间**（1:1）：先前把坐标直接塞进 `viewBox` 而 SVG 又被
 * `width: 100%` 撑满，于是浏览器按 viewBox 把整张图**放大**了好几倍——
 * 节点和文字一起变成巨型，标签被裁掉半边。修法是让 SVG 用固有尺寸。
 */
import type { MemoryGraphNode } from '@/api/memory'

/** 节点半径：底 7px，每个链接加 1.8px，最多加到 6 个链接。 */
const RADIUS_MIN = 7
 ...
 * 已知画布宽度时，最多把间距放大到几倍。
 *
 * 上限只有 1.6：放大间距是为了让中等规模的图用上空间，不是把节点摊到两端的角落
 * ——五六个节点的图拉满 1300px 之后，两个分量隔着半个屏幕对望，反而看不出谁和谁抱团。
 */
const MAX_SPREAD = 1.6

/**
 * 节点数不超过这个值时**全部标名字**，超过就只标度 ≥ 2 的。
 * 三五份记忆时不给名字的圆点等于没信息；几十份时全标就糊成一片。
 */
 ...
 * 算出整张图。
 *
 * **只画连上边的节点**：后端本来就不返回孤立节点，但这一层也自己保证——
 * 图要回答的是"结构"，一堆散点会把它淹掉。
 */
export function layoutGraph(
  ...
  // 每个分量的"半径"由它自己的层数决定；所有分量按最高那个对齐高度、各自垂直居中
  ...
  components.forEach((layers, componentAt) => {
    ...
    layers.forEach((layer, ring) => {
      ...
      layer.forEach((path, position) => {
        ...
        if (!node) return
        // 同层均匀铺开；每层加一个偏移，免得外圈的第一个点永远压在内圈的同一个角度上
        const angle =
