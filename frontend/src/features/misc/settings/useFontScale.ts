/**
 * 字号档位（《前端设计规范》§4 字阶 / 开发计划 §11.1）——与旧前端 `composables/useFontScale.ts` 对应。
 *
 * 存在的理由来自一条实际反馈：「web 字体偏小」。查下来**不是基线值低**，
 * 而是**分布**——小字号占满了界面外壳，正文反倒只是众多字号里的一个。
 * 所以这一轮做两件事：先把基线抬一档，再给用户一个可调档位。
 *
...
export interface FontScaleOption {
  name: FontScaleName
  label: string
  /** 中文字号阶梯**不用百分比表达**：写"15px"比"+7%"更能让人判断合不合适。 */
  bodySize: number
  hint: string
}
 ...
 *
 * 为什么要反推：两处各读一次，一旦有一处读到的不是同一个值，就会**渲染成一个档位、
 * 选择器高亮另一个档位**（实测踩到过）。以"已经生效的值"为唯一事实来源，
 * 这种分叉就不可能发生。
 */
function readApplied(): FontScaleName | null {
  ...
  return FONT_SCALES.find((item) => Math.abs(factorOf(item.name) - factor) < 0.001)?.name ?? null
}

let scale: FontScaleName = DEFAULT_SCALE
...
/** 把档位写成根节点上的 `--font-scale`；字阶各档在 tokens.css 里乘这个系数。 */
...
export function setFontScale(next: FontScaleName): void {
  applyFontScale(next)
  emit()
  try {
...
function snapshot(): FontScaleName {
  return scale
}
