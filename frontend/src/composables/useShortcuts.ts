 ...
 * 改之前全站只有一处硬编码的 Ctrl+K（`SideNav` 的新建会话），而界面上还摆着
 * 一枚 `Ctrl K` 的提示——用户改不了它、两块写法也对不上（一处 keydown、
 * 一处 kbd 标签，谁都不知道另一处存在）。ZCode 的做法是把快捷键做成**注册表**：
 * 命令（做什么）与绑定（按哪几个键）分开，绑定可改、可多个、可恢复默认，
 * 冲突当场提示。照这条抄，界面里就只剩"命令 → 干活"的映射。
 *
 * 四件事各有一处真相：
 *
 * - **命令表**（`SHORTCUT_COMMANDS`）：id、名字、作用域、默认绑定。改默认值只动这里；
 * - **绑定存 localStorage**（`kylab-shortcuts`）：与主题、字号、侧栏折叠同一族
 *   "这台机器怎么用"的偏好，不进后端（换台机器该重新选，它也不是知识库配置）；
 * - **匹配**（`matchShortcut`）：修饰键**必须精确对上**——不精确的话
 *   "Shift+Enter 换行"会被"Enter 发送"抢走，而那正是最容易被忽略的一类错；
 * - **冲突**（`conflictMessage`）：同一个绑定被两条命令用着就说出来，
 *   但**不拦着不让改**——ZCode 也是提示而非拒绝（用户完全可能先把 A 改走，
 *   再回来给 B 绑上；拦着不让改就会卡在这种中间态里）。
 */
...
/** 四条命令的 id（P2-1 先接这四条，够覆盖"打字、开新会话、看侧栏"这几件事）。 */
...
export interface ShortcutCommand {
  ...
  scope: ShortcutScope
  /**
   * 默认绑定，**可以有多条**（照 ZCode 的多绑定）：用户不必在
   * "我习惯的那组键"与"产品默认的那组键"之间二选一。
   */
  defaults: string[]
}
 ...
 * 命令表。
 *
 * 四条默认值里有三条是**界面本来就写着**的（回车发送、Shift+回车换行、
 * Ctrl/Cmd+K 新建会话，见 `SideNav` 的提示与 `ChatView` 的输入框），
 * 第四条（切换侧栏 Ctrl/Cmd+B）是这次补的：侧栏顶上那颗折叠按钮没有快捷键，
...
/** 绑定存这儿（本机偏好，与 `kylab-sidebar-collapsed` 同一族）。 */
...
/**
 * 主键的规范写法（用于**冲突比对**与**展示**两处，见 `canonicalBinding`）。
 *
 * 单字符键大写（`k` → `K`），其余保留 JS 的键名形状（`enter` → `Enter`，
 * `escape` → `Escape`）。**匹配不受它影响**：那一层两边都小写比较（见 `matchesEvent`），
 * 所以用户在设置里录出 `Enter` 还是 `enter` 都能用。
 */
 ...
 * 挑 Ctrl 是本仓既有的口径（`SideNav` 的提示一直是 `Ctrl K`）。
 * macOS 上按的仍然是 Cmd（匹配那一层认 metaKey，见 `matchesEvent`）。
 */
 ...
 *
 * 只按修饰键（Ctrl / Shift / Alt / Cmd）时返回 `null`：那不是"一组键"，
 * 而是用户正在往那组键上按——录下来会得到一条永远触发不了的绑定。
 */
...
function readStored(): BindingMap {
  ...
  try {
    ...
    for (const command of SHORTCUT_COMMANDS) {
      ...
      // 或者被手工改坏的内容——那种数据不该让整张表失效（其余命令照旧可用）
...
/** 当前的绑定表（单例：设置页与运行时读的是同一份）。 */
const bindings = ref<BindingMap>(typeof window === 'undefined' ? defaultsMap() : readStored())
...
function persist(): void {
  try {
    ...
    for (const command of SHORTCUT_COMMANDS) {
      const value = bindings.value[command.id] ?? []
...
export function conflictingCommand(id: ShortcutId, binding: string): ShortcutCommand | null {
  ...
  for (const command of SHORTCUT_COMMANDS) {
    ...
    if (bindingsOf(command.id).some((item) => canonicalBinding(item) === canonical)) {
      return command
    }
  }
  return null
 ...
 * 冲突提示文案（有冲突时非空）。
 *
 * 措辞照 ZCode 的冲突提示：**先说是谁占着**——只说"冲突了"用户还得自己找是跟谁撞了，
 * 而设置页上一屏就只有这几行。
 */
 ...
 *    同一组键在两个作用域里各有一条命令是合法的（比如输入框里的回车与全局的回车）；
 ...
 * 是删行、`Cmd+B` 是加粗（笔记页的富文本编辑器两样都有）。
 * 这条判断在 `SideNav` 里原本就有（为 Ctrl+K 写的），这里只是搬进注册表，
 * 让"全局"这个词的边界**只在这一处**定义。
 */
